		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} foolish member links are not allowed here \`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} has been kicked for sending links this should be a warning for others\`\`\``,
					mentions: [sender]
				});
				break;
